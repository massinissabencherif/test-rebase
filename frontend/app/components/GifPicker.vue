<template>
  <div style="display:inline-block;">
    <button
      type="button"
      @click="open = true"
      style="font-family:'Courier New',monospace;font-size:10px;letter-spacing:1px;text-transform:uppercase;background:none;border:1px solid #333;color:#666;padding:6px 12px;cursor:pointer;display:flex;align-items:center;gap:6px;transition:border-color 0.15s,color 0.15s;"
      ...
      <div style="background:#1e1e1e;border:1px solid #333;border-top:2px solid #e02020;width:100%;max-width:540px;max-height:82vh;display:flex;flex-direction:column;overflow:hidden;">
        ...
        <div style="padding:12px 16px;border-bottom:1px solid #2a2a2a;display:flex;align-items:center;gap:10px;">
          ...
          <div style="flex:1;position:relative;">
            <input
              ...
              style="width:100%;background:#252525;border:1px solid #333;color:#d8d8d8;font-family:'Courier New',monospace;font-size:12px;padding:7px 30px 7px 10px;outline:none;transition:border-color 0.15s;"
              class="focus:border-[#e02020]"
            />
            <button
              v-if="query"
              @click="query = ''; if (configured) loadTrending()"
              style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;color:#555;cursor:pointer;font-size:14px;line-height:1;"
            >×</button>
          </div>
          <button @click="close" style="background:none;border:none;color:#555;cursor:pointer;font-size:20px;line-height:1;flex-shrink:0;" class="hover:text-[#fff]">×</button>
        </div>
        ...
        <template v-if="configured">
          <div style="padding:5px 16px;background:#171717;border-bottom:1px solid #222;display:flex;align-items:center;gap:6px;">
            <span style="font-family:'Courier New',monospace;font-size:8px;letter-spacing:2px;color:#444;text-transform:uppercase;">Powered by</span>
            <span style="font-family:impact,sans-serif;font-size:11px;color:#555;letter-spacing:1px;">GIPHY</span>
          </div>

          <div style="flex:1;overflow-y:auto;padding:8px;" class="gif-scroll">
            <div v-if="loading" style="display:flex;justify-content:center;padding:32px;font-family:'Courier New',monospace;font-size:11px;color:#555;">Recherche…</div>
            ...
            <div v-else-if="!loading" style="text-align:center;padding:32px;font-family:'Courier New',monospace;font-size:11px;color:#555;">
          ...
          <div style="padding:10px 16px;border-top:1px solid #2a2a2a;display:flex;align-items:center;gap:12px;">
            <div v-if="selected" style="display:flex;align-items:center;gap:8px;flex:1;min-width:0;">
              ...
              <span style="font-family:'Courier New',monospace;font-size:10px;color:#888;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ selected.title }}</span>
            </div>
            <div v-else style="flex:1;font-family:'Courier New',monospace;font-size:10px;color:#444;">Sélectionne un GIF</div>
            <button @click="confirmGiphy" :disabled="!selected" class="btn-primary" style="font-size:10px;padding:7px 16px;flex-shrink:0;">Insérer</button>
          </div>
        </template>
        ...
        <template v-else>
          <div style="padding:14px 16px;background:#171717;border-bottom:1px solid #222;">
            <p style="font-family:'Courier New',monospace;font-size:10px;line-height:1.7;color:#666;margin-bottom:8px;">
            ...
            <p style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:1px;color:#444;">Sur Giphy : clic droit sur le GIF → « Copier l'adresse de l'image »</p>
          </div>
          ...
          <div style="flex:1;padding:16px;display:flex;flex-direction:column;gap:12px;">
            ...
            <div v-else-if="query && !urlPreviewOk" style="font-family:'Courier New',monospace;font-size:10px;color:#666;text-align:center;padding:16px;">
              URL non reconnue comme image. Assure-toi de copier l'URL directe du GIF (.gif, .webp…)
            </div>
            <div v-else style="font-family:'Courier New',monospace;font-size:10px;color:#444;text-align:center;padding:24px;">
          ...
          <div style="padding:10px 16px;border-top:1px solid #2a2a2a;display:flex;justify-content:flex-end;">
            <button @click="confirmUrl" :disabled="!urlPreviewOk" class="btn-primary" style="font-size:10px;padding:7px 16px;">Insérer</button>
          </div>
        </template>
