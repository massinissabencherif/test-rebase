<template>
  <div>
    ...
    <div style="border-bottom:1px solid #2a2a2a;">
      <div class="max-w-[860px] mx-auto px-6 pt-9 pb-0">
        ...
        <div style="display:flex;align-items:flex-end;justify-content:space-between;padding-bottom:0;">
          ...
          <button v-if="activeTab === 'mine'" @click="showCreate = true" class="btn-primary" style="font-size:11px;padding:9px 20px;margin-bottom:8px;">
            + NOUVELLE LISTE
          </button>
        </div>
        ...
        <div style="display:flex;margin-top:20px;">
          <button
            ...
            style="font-family:'Courier New',monospace;font-size:10px;letter-spacing:3px;text-transform:uppercase;padding:10px 24px;background:transparent;border:none;border-bottom:2px solid transparent;cursor:pointer;transition:all 0.15s;"
            :style="activeTab === 'mine' ? 'color:#fff;border-bottom-color:#e02020;' : 'color:#555;'"
            class="hover:text-[#aaa]"
          >Mes listes</button>
          <button
            @click="switchToPublic"
            style="font-family:'Courier New',monospace;font-size:10px;letter-spacing:3px;text-transform:uppercase;padding:10px 24px;background:transparent;border:none;border-bottom:2px solid transparent;cursor:pointer;transition:all 0.15s;"
            :style="activeTab === 'public' ? 'color:#fff;border-bottom-color:#e02020;' : 'color:#555;'"
            class="hover:text-[#aaa]"
          >Publiques</button>
        </div>
      </div>
    ...
    <div class="max-w-[860px] mx-auto px-6 py-8">
      ...
      <Teleport to="body">
        <div v-if="showCreate" style="position:fixed;inset:0;z-index:50;display:flex;align-items:center;justify-content:center;padding:16px;">
          ...
          <div style="position:relative;background:#111;border:1px solid #2a2a2a;border-top:2px solid #e02020;padding:28px;width:100%;max-width:420px;">
            ...
            <div v-if="createError" style="font-family:'Courier New',monospace;font-size:11px;color:#e02020;margin-bottom:12px;">{{ createError }}</div>
      ...
      <template v-if="activeTab === 'mine'">
        ...
        <div v-else-if="!lists.length" style="text-align:center;padding:80px 0;">
          <p style="font-family:impact,sans-serif;font-size:18px;letter-spacing:2px;text-transform:uppercase;color:#555;margin-bottom:8px;">Aucune liste</p>
        ...
        <template v-else>
          ...
          <div style="position:relative;margin-bottom:20px;">
            ...
            />
            <svg style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:#555;" width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
          ...
          <div v-if="!filteredMyLists.length" style="font-family:'Courier New',monospace;font-size:11px;color:#555;padding:40px 0;text-align:center;">
          ...
          <div v-else style="display:flex;flex-direction:column;gap:1px;background:#2a2a2a;">
            ...
            >
              ...
              <p v-if="list.description" style="font-family:'Courier New',monospace;font-size:11px;line-height:1.6;color:#777;margin-bottom:8px;">{{ list.description }}</p>
              <div style="display:flex;align-items:center;gap:16px;">
                <span style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#555;">
                  {{ list._count.items }} comic{{ list._count.items !== 1 ? 's' : '' }}
                </span>
                <NuxtLink :to="`/lists/${list.id}`" style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#aaa;text-decoration:none;transition:color 0.15s;" class="hover:text-[#e02020]">
                  Voir →
                </NuxtLink>
                <button
                  v-if="list.isPublic"
                  @click="copyLink(list.slug)"
                  style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#aaa;background:none;border:none;cursor:pointer;transition:color 0.15s;"
                  class="hover:text-[#e02020]"
                >Copier le lien</button>
                <button
                  @click="deleteList(list)"
                  style="margin-left:auto;font-family:'Courier New',monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#555;background:none;border:none;cursor:pointer;transition:color 0.15s;"
                  class="hover:text-[#e02020]"
                >Supprimer</button>
              </div>
            </div>
      ...
      <template v-if="activeTab === 'public'">
        ...
        <div v-if="publicPending" style="display:flex;align-items:center;gap:10px;font-family:'Courier New',monospace;font-size:11px;letter-spacing:2px;color:#888;text-transform:uppercase;padding:80px 0;">
          <svg class="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
          </svg>
        ...
        <div v-else-if="!publicLists.length" style="text-align:center;padding:80px 0;">
          <p style="font-family:'Courier New',monospace;font-size:11px;letter-spacing:2px;color:#444;text-transform:uppercase;">
            {{ publicSearch ? 'Aucun résultat.' : 'Aucune liste publique pour l\'instant.' }}
          </p>
        </div>

        <template v-else>
          <div style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#555;margin-bottom:16px;">
          ...
          <div style="display:flex;flex-direction:column;gap:1px;background:#2a2a2a;">
            ...
            >
              <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:6px;">
                ...
                <span style="flex-shrink:0;font-family:'Courier New',monospace;font-size:8px;letter-spacing:2px;text-transform:uppercase;padding:4px 10px;border:1px solid;border-color:rgba(34,197,94,0.4);background:rgba(34,197,94,0.08);color:#86efac;">🌐 PUBLIC</span>
              </div>
              <p style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:1px;color:#666;margin-bottom:6px;">
                par <span style="color:#999;">{{ list.user.username }}</span>
              </p>
              <p v-if="list.description" style="font-family:'Courier New',monospace;font-size:11px;line-height:1.6;color:#777;margin-bottom:8px;">{{ list.description }}</p>
              <div style="display:flex;align-items:center;gap:16px;">
                <span style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#555;">
                ...
                <NuxtLink :to="`/lists/public/${list.slug}`" style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#aaa;text-decoration:none;transition:color 0.15s;" class="hover:text-[#e02020]">
                  Voir →
                </NuxtLink>
              </div>
            </div>
    ...
    <Teleport to="body">
      <div
        v-if="copied"
        style="position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:12px 24px;background:rgba(34,197,94,0.15);border:1px solid rgba(34,197,94,0.3);color:#86efac;font-family:'Courier New',monospace;font-size:11px;letter-spacing:3px;text-transform:uppercase;z-index:100;"
      >
        Lien copié ✓
      </div>
    </Teleport>
