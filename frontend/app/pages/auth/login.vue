    ...
    <div class="relative w-full max-w-sm">
      ...
      <div class="text-center mb-8">
        ...
        <template v-if="step !== '2fa'">
          <h1 class="text-2xl font-bold mb-1">Ton univers comics t'attend</h1>
          <p class="text-gray-500 text-sm mb-4">Suis ta lecture, note tes comics, partage tes listes.</p>
          <div class="flex justify-center gap-4 text-xs text-gray-600 mb-6">
            <span>📚 Journal de lecture</span>
            <span>⭐ Notes & avis</span>
            <span>🗂️ Listes perso</span>
          </div>
        </template>
        <template v-else>
          <h1 class="text-2xl font-bold mb-1">Vérification 2FA</h1>
          <p class="text-gray-500 text-sm">Entre le code de ton application</p>
        </template>
      </div>

      <!-- 2FA step -->
      <div v-if="step === '2fa'" class="card p-7">
        <form @submit.prevent="submit2FA" class="space-y-4" aria-label="Vérification 2FA">
          <div>
            <label for="totp-code" class="block text-xs font-medium text-gray-400 mb-2">Code d'authentification</label>
            <input
              id="totp-code"
              v-model="totpCode"
              type="text"
          ...
          <div v-if="error" role="alert" class="flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3 text-sm text-red-400">
      ...
      <div v-else class="card p-7">
        <form @submit.prevent="submit" class="space-y-4" aria-label="Connexion par email">
          <div>
            <label for="login-email" class="block text-xs font-medium text-gray-400 mb-2">Email</label>
            <input id="login-email" v-model="form.email" type="email" required placeholder="toi@example.com" class="input" autocomplete="email" />
          </div>

          <div>
            <label for="login-password" class="block text-xs font-medium text-gray-400 mb-2">Mot de passe</label>
